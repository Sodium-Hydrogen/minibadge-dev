...
const char _help1[] PROGMEM = " HZ:\t\t Sets a clock on MB_CLK to pulse at a 50% duty cycle set at HZ (0 to turn it off. Max of 20 Hz.)";
...
void setup() {
  ...
  digitalWrite(MB_CLK, HIGH);
  delay(30);
  digitalWrite(MB_CLK, LOW);
...
void loop() {
  ...
  if( clk_hz != 0 && millis() > ( time_since_last_pulse + (uint32_t)(500/clk_hz) )){
    ...
    digitalWrite(MB_CLK, !digitalRead(MB_CLK));
  }
}
...
void read_device(uint8_t addr, bool getWriteState){
  ...
  if(Wire.endTransmission(addr) == 0){ // Check if device is on bus before beginning read.
    if(getWriteState){
      ...
      Serial.print( res?F(" supports"):F(" does not support"));
...
void write_device(uint8_t addr, uint8_t command, uint8_t argc, uint8_t * args){
  uint8_t res;
  Wire.beginTransmission(addr);
  if(Wire.endTransmission(addr) == 0){ // Check if device is on bus before beginning read.
    ...
    if(res){
      ...
      for(uint8_t i = 0; i < argc; i++){
        ...
        Serial.print((uint8_t)args[i], HEX);
        Wire.write((uint8_t)args[i]);
      }
      Serial.println("");
      Wire.endTransmission();
    }
...
void set_clock(String com){
    ...
    Serial.print(F("Setting MB_CLK to clock speed of "));
    Serial.print(clk_hz);
    Serial.println(F(" Hz."));
    pinMode(MB_CLK, OUTPUT);
    digitalWrite(MB_CLK, LOW);
